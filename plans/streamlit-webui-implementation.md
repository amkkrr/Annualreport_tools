---
task_id: streamlit-webui-implementation
type: FEAT
complexity: S
current_phase: I.G
completed_phases: [P0, P0.B, P0.C, P2.G, P2.R, P2.A]
branch: feature/streamlit-webui

next_action: CALL_GEMINI_IG
next_action_prompt: |
  根据设计规格书生成 Streamlit WebUI 代码。

  文件结构:
  - webui/app.py - 主入口
  - webui/pages/1_📊_监控仪表盘.py
  - webui/pages/2_⚙️_配置管理.py
  - webui/pages/3_🚀_任务管理.py
  - webui/components/__init__.py
  - webui/components/db_utils.py
  - webui/components/config_editor.py
  - webui/components/task_runner.py
  - webui/.streamlit/config.toml
  - webui/requirements.txt

  参考规格书: @plans/streamlit-webui-implementation.md

blocked: false
blocked_reason: ""

created_at: 2026-01-11
updated_at: 2026-01-11
---

# Streamlit WebUI 实现计划

> 基于调研报告 `plans/streamlit-webui-research.md` 的实现任务

## 问题定义

**类型**: FEAT-S (标准功能开发)

**背景**: 调研报告已确认 Streamlit 是最佳技术选型。现在需要实现 WebUI，提供参数配置、进度监控和任务管理功能，提升工具易用性。

**范围**:

### 做什么
1. 创建 `webui/` 目录，搭建 Streamlit 多页应用骨架
2. 实现监控仪表盘页（复用 DuckDB 视图展示进度）
3. 实现配置管理页（读写 `config.yaml`）
4. 实现任务管理页（后台进程启停、日志查看）
5. 添加基础测试

### 不做什么
- 不实现用户认证（P3 优先级，本次迭代不做）
- 不实现复杂的任务队列系统（使用简单的 subprocess）
- 不做 Docker 化部署（后续迭代）

**完成标准**:
1. `streamlit run webui/app.py` 可正常启动
2. 监控仪表盘页可展示 `reports_progress` 等视图数据
3. 配置管理页可读取、编辑、保存 `config.yaml`
4. 任务管理页可启动/停止爬虫、下载、提取任务
5. 基础测试通过

---

## 设计规格书 (P2.G + P2.R)

> Gemini 生成初稿，Claude 审核完善

### 1. 文件结构

```
webui/
├── app.py                      # 主应用入口
├── .streamlit/
│   └── config.toml             # Streamlit 配置
├── pages/
│   ├── 1_📊_监控仪表盘.py       # 进度监控页
│   ├── 2_⚙️_配置管理.py         # 配置编辑页
│   └── 3_🚀_任务管理.py         # 任务控制页
├── components/
│   ├── __init__.py
│   ├── db_utils.py             # DuckDB 查询封装
│   ├── config_editor.py        # config.yaml 读写
│   └── task_runner.py          # 后台任务管理
└── requirements.txt            # 依赖
```

### 2. 页面详细设计

#### 2.1 监控仪表盘页 (`1_📊_监控仪表盘.py`)

**布局**:
```
┌─────────────────────────────────────────────────────────┐
│ 📊 监控仪表盘                            [🔄 刷新数据] │
├─────────────────────────────────────────────────────────┤
│ ┌─────────┐  ┌─────────┐  ┌─────────┐  ┌─────────┐     │
│ │ 待下载  │  │ 待转换  │  │ 待审核  │  │ 已完成  │     │
│ │   127   │  │   45    │  │   12    │  │  1,234  │     │
│ └─────────┘  └─────────┘  └─────────┘  └─────────┘     │
├─────────────────────────────────────────────────────────┤
│ 年度处理进度                                            │
│ ┌─────────────────────────────────────────────────────┐ │
│ │ year | total | downloaded | converted | extracted  │ │
│ │ 2023 | 5000  | 4500       | 4200      | 4000       │ │
│ └─────────────────────────────────────────────────────┘ │
├─────────────────────────────────────────────────────────┤
│ [待下载队列] [待转换队列] [待审核 MDA]                  │
│ ┌─────────────────────────────────────────────────────┐ │
│ │ stock_code | short_name | year | url               │ │
│ └─────────────────────────────────────────────────────┘ │
└─────────────────────────────────────────────────────────┘
```

**组件**:
- `st.metric`: 关键指标（待下载、待转换、待审核、已完成）
- `st.dataframe`: 年度进度表、待处理队列
- `st.tabs`: 切换不同队列视图
- `st.button`: 刷新数据

#### 2.2 配置管理页 (`2_⚙️_配置管理.py`)

**布局**:
```
┌─────────────────────────────────────────────────────────┐
│ ⚙️ 配置管理                                             │
├─────────────────────────────────────────────────────────┤
│ ┌─ 爬虫配置 ─────────────────────────────────────────┐  │
│ │ 目标年份: [2023] [2022] [2021]                     │  │
│ │ 板块: [✓ 沪市主板] [✓ 深市主板] [ ] 创业板        │  │
│ │ 排除关键词: [摘要] [半年报] [季报]                 │  │
│ └────────────────────────────────────────────────────┘  │
│ ┌─ 下载配置 ─────────────────────────────────────────┐  │
│ │ 并发进程数: [4]                                    │  │
│ │ [✓] 下载后删除 PDF  [ ] 跳过已存在                 │  │
│ └────────────────────────────────────────────────────┘  │
│ ┌─ MDA 提取配置 ─────────────────────────────────────┐  │
│ │ 工作进程数: [4]                                    │  │
│ │ [✓] 增量模式                                       │  │
│ └────────────────────────────────────────────────────┘  │
│                                        [💾 保存配置]    │
└─────────────────────────────────────────────────────────┘
```

**组件**:
- `st.form`: 包裹所有配置项，避免频繁刷新
- `st.multiselect`: 年份、板块选择
- `st.number_input`: 并发数
- `st.checkbox`: 布尔选项
- `st.text_area`: 排除关键词（每行一个）

#### 2.3 任务管理页 (`3_🚀_任务管理.py`)

**布局**:
```
┌─────────────────────────────────────────────────────────┐
│ 🚀 任务管理                                             │
├─────────────────────────────────────────────────────────┤
│ ┌─ 爬取链接 ────────┐ ┌─ 下载转换 ────────┐ ┌─ MDA ─┐  │
│ │ 状态: ● 已停止    │ │ 状态: ● 运行中    │ │ ● 停止│  │
│ │ [▶ 启动] [■ 停止] │ │ [▶ 启动] [■ 停止] │ │ [▶][■]│  │
│ └───────────────────┘ └───────────────────┘ └───────┘  │
├─────────────────────────────────────────────────────────┤
│ 实时日志                                   [自动刷新 ✓] │
│ ┌─────────────────────────────────────────────────────┐ │
│ │ [2026-01-11 10:30:15] 开始下载 600519_贵州茅台...  │ │
│ │ [2026-01-11 10:30:16] 下载完成                     │ │
│ │ [2026-01-11 10:30:17] 开始转换...                  │ │
│ └─────────────────────────────────────────────────────┘ │
└─────────────────────────────────────────────────────────┘
```

**组件**:
- `st.columns`: 并排显示任务控制区
- `st.button`: 启动/停止按钮
- `st.status`: 任务状态指示
- `st.code`: 日志显示
- `st.checkbox`: 自动刷新开关

### 3. 核心组件接口

#### 3.1 `components/db_utils.py`

```python
from __future__ import annotations
import streamlit as st
import duckdb
import pandas as pd
from pathlib import Path

DEFAULT_DB_PATH = Path("data/annual_reports.duckdb")

@st.cache_resource
def get_connection(db_path: Path = DEFAULT_DB_PATH) -> duckdb.DuckDBPyConnection:
    """获取 DuckDB 只读连接（缓存）"""
    return duckdb.connect(str(db_path), read_only=True)

@st.cache_data(ttl=60)
def get_reports_progress(_conn: duckdb.DuckDBPyConnection) -> pd.DataFrame:
    """查询年度处理进度"""
    return _conn.execute("SELECT * FROM reports_progress").df()

@st.cache_data(ttl=60)
def get_pending_downloads(_conn: duckdb.DuckDBPyConnection) -> pd.DataFrame:
    """查询待下载队列"""
    return _conn.execute("SELECT * FROM pending_downloads LIMIT 100").df()

@st.cache_data(ttl=60)
def get_pending_converts(_conn: duckdb.DuckDBPyConnection) -> pd.DataFrame:
    """查询待转换队列"""
    return _conn.execute("SELECT * FROM pending_converts LIMIT 100").df()

@st.cache_data(ttl=60)
def get_mda_needs_review(_conn: duckdb.DuckDBPyConnection) -> pd.DataFrame:
    """查询待审核 MDA"""
    return _conn.execute("SELECT * FROM mda_needs_review LIMIT 100").df()

def get_counts(_conn: duckdb.DuckDBPyConnection) -> dict[str, int]:
    """获取各队列计数"""
    return {
        "pending_downloads": _conn.execute("SELECT COUNT(*) FROM pending_downloads").fetchone()[0],
        "pending_converts": _conn.execute("SELECT COUNT(*) FROM pending_converts").fetchone()[0],
        "mda_needs_review": _conn.execute("SELECT COUNT(*) FROM mda_needs_review").fetchone()[0],
        "total_extracted": _conn.execute(
            "SELECT COUNT(*) FROM reports WHERE extract_status = 'success'"
        ).fetchone()[0],
    }
```

#### 3.2 `components/config_editor.py`

```python
from __future__ import annotations
from pathlib import Path
import yaml

DEFAULT_CONFIG_PATH = Path("config.yaml")

def load_config(config_path: Path = DEFAULT_CONFIG_PATH) -> dict:
    """读取 config.yaml"""
    if not config_path.exists():
        return {}
    with open(config_path, encoding="utf-8") as f:
        return yaml.safe_load(f) or {}

def save_config(config_data: dict, config_path: Path = DEFAULT_CONFIG_PATH) -> None:
    """保存 config.yaml"""
    with open(config_path, "w", encoding="utf-8") as f:
        yaml.dump(config_data, f, allow_unicode=True, default_flow_style=False)
```

#### 3.3 `components/task_runner.py`

```python
from __future__ import annotations
import subprocess
import signal
from pathlib import Path
from dataclasses import dataclass
from typing import Literal

TaskName = Literal["crawler", "converter", "extractor"]

TASK_SCRIPTS = {
    "crawler": "1.report_link_crawler.py",
    "converter": "2.pdf_batch_converter.py",
    "extractor": "mda_extractor.py",
}

LOG_DIR = Path("logs/webui")

@dataclass
class TaskState:
    process: subprocess.Popen | None = None
    log_file: Path | None = None

def start_task(task_name: TaskName, extra_args: list[str] | None = None) -> TaskState:
    """启动后台任务"""
    LOG_DIR.mkdir(parents=True, exist_ok=True)
    log_file = LOG_DIR / f"{task_name}.log"

    script = TASK_SCRIPTS[task_name]
    cmd = ["python", script, "--use-config"]
    if extra_args:
        cmd.extend(extra_args)

    with open(log_file, "w", encoding="utf-8") as f:
        process = subprocess.Popen(
            cmd,
            stdout=f,
            stderr=subprocess.STDOUT,
            cwd=Path.cwd(),
        )

    return TaskState(process=process, log_file=log_file)

def stop_task(state: TaskState) -> None:
    """停止任务"""
    if state.process and state.process.poll() is None:
        state.process.terminate()
        try:
            state.process.wait(timeout=5)
        except subprocess.TimeoutExpired:
            state.process.kill()

def get_task_status(state: TaskState) -> str:
    """获取任务状态"""
    if state.process is None:
        return "stopped"
    exit_code = state.process.poll()
    if exit_code is None:
        return "running"
    return "completed" if exit_code == 0 else "error"

def read_log(state: TaskState, tail_lines: int = 50) -> str:
    """读取日志尾部"""
    if state.log_file is None or not state.log_file.exists():
        return ""
    lines = state.log_file.read_text(encoding="utf-8", errors="replace").splitlines()
    return "\n".join(lines[-tail_lines:])
```

### 4. 状态管理方案

使用 `st.session_state` 管理任务进程状态：

```python
# 在 3_🚀_任务管理.py 开头初始化
if "task_states" not in st.session_state:
    st.session_state.task_states = {
        "crawler": TaskState(),
        "converter": TaskState(),
        "extractor": TaskState(),
    }
```

### 5. 错误处理

| 场景 | 处理方式 |
|------|----------|
| DuckDB 连接失败 | `st.error("数据库连接失败")`，返回空 DataFrame |
| config.yaml 不存在 | `st.warning("配置文件不存在")`，使用默认值 |
| 任务启动失败 | `st.error("启动失败: {error}")`，记录异常 |
| 任务异常退出 | 状态显示 "错误"，提示查看日志 |

### 6. 验收标准

| ID | Given | When | Then |
|----|-------|------|------|
| W1 | webui/ 目录存在 | `streamlit run webui/app.py` | 应用启动，显示首页 |
| W2 | DuckDB 有 5 条待下载记录 | 访问监控页 | "待下载" 指标显示 5 |
| W3 | config.yaml 中 processes=4 | 修改为 8 并保存 | 文件更新，显示成功提示 |
| W4 | 所有任务停止 | 点击"启动爬取" | 状态变为"运行中"，后台进程启动 |
| W5 | 爬取任务运行中 | 点击"停止" | 状态变为"已停止"，进程终止 |
