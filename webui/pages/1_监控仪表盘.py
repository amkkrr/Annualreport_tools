...
if conn:
    # Key Metrics
    counts = db_utils.get_counts(conn)
    st.subheader("关键指标")
    ...
    # Annual Progress
    st.subheader("年度处理进度")
    progress_df = db_utils.get_reports_progress(conn)
    ...
    with tab1:
        df = db_utils.get_pending_downloads(conn)
        if not df.empty:
    ...
    with tab2:
        df = db_utils.get_pending_converts(conn)
        if not df.empty:
            st.dataframe(df, use_container_width=True, height=350)
        else:
            st.success("没有待转换的任务。")
    ...
    with tab3:
        df = db_utils.get_mda_needs_review(conn)
        if not df.empty:
            st.dataframe(df, use_container_width=True, height=350)
        else:
    ...
    st.error("数据库未连接，无法显示仪表盘。请确认数据库文件存在。")
