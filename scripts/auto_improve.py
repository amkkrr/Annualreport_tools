...
def analyze_failure_pattern(result: dict, golden_sample: dict | None) -> str:
    """分析单个样本的失败模式"""
...
def analyze_sample_with_llm(
    sample_id: str, golden_sample: dict, txt_path: str, timeout: int = 120
) -> dict | None:
    """使用 LLM 分析单个失败样本的原因"""
...
def main():
    ...
    parser.add_argument(
        "--eval", type=str, default="data/evaluation_results.json", help="评估结果文件"
    )
    parser.add_argument(
        "--golden", type=str, default="data/golden_set_draft.json", help="Golden 数据集文件"
    )
    parser.add_argument("--apply", action="store_true", help="应用改进并重新评估")
    parser.add_argument("--use-llm", action="store_true", help="使用 LLM 分析低分样本 (较慢)")
    parser.add_argument(
        "--output", type=str, default="data/improvement_report.json", help="输出报告路径"
    )
    ...
    if args.use_llm:
        ...
        # 选择低分样本进行分析
        low_score_samples = [
            r for r in eval_data.get("results", []) if r.get("final_score", 0) < 30
        ][:5]  # 最多分析 5 个
    ...
    report = {
        "patterns": {
            k: {"count": v.count, "avg_score": v.avg_score, "sample_ids": v.sample_ids}
