...
- [ ] `P0` 创建 `config.yaml` 模板，集中管理年份、路径、数据库、关键词配置
- [ ] `P0` 使用 Pydantic 定义配置类，替代手写验证层（一步实现加载+校验）
- [ ] `P1` 改造 `mda_extractor.py` 以支持读取 `config.yaml`
- [ ] `P2` 对危险配置增加校验（如输出路径不能为根目录、不能覆盖源文件）
...
| ID | 验收项 | 指标 |
|----|--------|------|
| M1-Q1 | 迁移性能 | 5 万条数据迁移耗时 < 60s |
| M1-Q2 | 迁移幂等 | 重复执行迁移脚本不产生重复数据 |
| M1-Q3 | 配置热加载 | 修改 `config.yaml` 后无需重启即可生效（可选） |
...
- [ ] `P0` 人工标注 50-100 份年报的 MD&A 边界（起止页码/段落），作为评估基准
- [ ] `P1` 编写评估脚本，计算提取结果与标注的 IoU / F1 分数
