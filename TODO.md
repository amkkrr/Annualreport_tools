...
- [x] `P0` **架构设计**：确定 SQLite (OLTP) 与 DuckDB (OLAP) 的表拆分方案，明确 **SQLite 为元数据唯一写入点**（完成于 2026-01-12）
- [x] `P0` **SQLite 集成**：改造 `db.py`，引入 SQLite 连接管理，开启 WAL 模式支持并发（完成于 2026-01-12）
- [x] `P0` **DuckDB 连接策略**：
...
- [x] `P0` **数据迁移**：将 `companies`, `reports`, `llm_call_logs` 等高频读写表迁移至 SQLite（完成于 2026-01-12）
- [x] `P1` **DuckDB 联邦查询**：配置 DuckDB 自动挂载 SQLite (`ATTACH 'metadata.db'`)，保持分析脚本兼容性（完成于 2026-01-12）
