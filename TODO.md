...
## Phase 1.5: 架构升级 — 双数据库并发治理

> 目标：引入 SQLite 作为 OLTP 前置库，解决 DuckDB 单写锁导致的 WebUI 与爬虫并发冲突。**原则：本地 Python 程序（爬虫、WebUI）仅读写 SQLite，禁止直接连接 DuckDB（分析模块除外）。**

### 1.5.1 双数据库架构落地

- [ ] `P0` **架构设计**：确定 SQLite (OLTP) 与 DuckDB (OLAP) 的表拆分方案，明确 **SQLite 为元数据唯一写入点**
- [ ] `P0` **SQLite 集成**：改造 `db.py`，引入 SQLite 连接管理，开启 WAL 模式支持并发
- [ ] `P0` **DuckDB 连接策略**：
    - 提取器（Writer）：实现短连接 + 重试机制（`duckdb.connect(..., config={'access_mode': 'READ_WRITE'})`）
    - 分析器（Reader）：强制使用只读连接（`config={'access_mode': 'READ_ONLY'}`），避免阻塞写入
- [ ] `P0` **数据迁移**：将 `companies`, `reports`, `llm_call_logs` 等高频读写表迁移至 SQLite
- [ ] `P1` **DuckDB 联邦查询**：配置 DuckDB 自动挂载 SQLite (`ATTACH 'metadata.db'`)，保持分析脚本兼容性

### 1.5.2 模块适配

- [ ] `P1` **爬虫适配**：`1.report_link_crawler.py` 改为**仅写入 SQLite**，移除所有 DuckDB 连接代码
- [ ] `P1` **WebUI 适配**：状态查询组件改为**仅读取 SQLite**，彻底解除对 DuckDB 的文件锁占用
- [ ] `P1` **并发验证**：验证爬虫运行期间 WebUI 可正常刷新状态
...
- [x] `P0` **LLM 初标 + 人工抽检**：使用 Claude/GPT 初步识别 MD&A 边界，人工仅审核/修正高不确定性样本（完成于 2026-01-10）
- [x] `P1` **LLM-as-Judge 评估器**：用强模型（如 Claude Opus）直接评估提取质量，输出完整性/准确性评分 + 理由（完成于 2026-01-10）
- [x] `P1` 编写评估脚本，支持 LLM 评分 + 关键特征校验（标志性标题存在、财务表格排除、长度合理）（完成于 2026-01-10）

### 2.2 精准提取策略升级

- [x] `P0` **策略优化迭代**：通过黄金数据集评估驱动，平均分从 68.86 提升至 96.51（完成于 2026-01-10）
- [x] `P1` **目录页解析 (TOC Parsing)**：实现 TOC 与正文提取结果的交叉校验，不一致时添加 FLAG_TOC_MISMATCH（完成于 2026-01-10）
- [x] `P2` **PDF 书签 (Outlines)**：在 PDF 转换阶段读取内置书签，附加到 PDFConversionResult（完成于 2026-01-10）
- [x] `P2` **字段切分**：实现 section_splitter.py，将 MD&A 切分为经营回顾与未来展望子字段（完成于 2026-01-10）

### 2.3 质量评分闭环

- [x] `P1` 升级评分器 `scorer.py`，增加负向特征检测（表格残留、页眉干扰、乱码比例等）（完成于 2026-01-10）
- [x] `P1` 建立反馈机制：数据库记录 `quality_score`，对低分样本标记 `needs_review`（完成于 2026-01-10）

### 📍 里程碑 M2: 全流程 DuckDB 驱动 + 质量评估体系上线 ✅ (验收通过 2026-01-11)

> 验收报告: [plans/M2_verification_report.md](plans/M2_verification_report.md)
...
| ID    | 验收项     | 指标                                   | 状态                                                     |
| ----- | ---------- | -------------------------------------- | -------------------------------------------------------- |
| M2-Q1 | 提取准确率 | 黄金集 F1 ≥ 0.85                       | ⚠️ F1=0（边界匹配问题），但平均分 96.51，文本重叠率 75.8% |
...
> 验收报告: [plans/M2.5_verification_report.md](plans/M2.5_verification_report.md) (待创建)
...
| M2.5-02 | 提取单测    | Mock 数据准备完毕         | 执行 `pytest tests/test_strategies.py tests/test_mda_extractor.py` | 含/不含目录样本均正确处理            | ✅    |
...
| **M1.5**   | 双数据库并发架构                  | M1              | 2 功能 + 0 质量   | ⏳ 待验收         |
